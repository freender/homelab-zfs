...
destination_pool="cache"  #this is the zpool in which your destination dataset will be created
...
child_destination_dataset="appdata"
...
zfs_replication() {
  ...
  if [ "$replication" = "zfs" ]; then
    ...
    /usr/sbin/syncoid "${syncoid_flags[@]}" "${source_path}" "${destination}"
    if [ $? -eq 0 ]; then
      if [ "$destination_remote" = "yes" ]; then
